...
function syncMarkerToMap(mapInstance, latlng) {
...
function setupExpandMap() {
    ...
    expandBtn.addEventListener('click', function() {
        ...
        if (!expandedMap) {
            expandedMap = L.map('expandedMap').setView([20.5937, 78.9629], 13);
            L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                attribution: '© OpenStreetMap contributors',
                maxZoom: 19,
                minZoom: 3
    ...
    fetch(url)
        ...
        .then(data => {
            if (data.address) {
                ...
                if (street1) street1.value = data.address.road || '';
                if (area1) area1.value = data.address.suburb || data.address.neighbourhood || '';
                if (city) city.value = data.address.city || data.address.town || '';
            }
        })
